...
async function main() {
  ...
  const sakuraRhod10Normal = await prisma.product.create({
    data: {
      ...
      baziElement: 'FIRE',
      metaphysicalProperties: JSON.stringify(['healing', 'love', 'compassion', 'emotional_balance']),
  ...
  const blueMoon8Normal = await prisma.product.create({
    data: {
      ...
      baziElement: 'WATER',
      metaphysicalProperties: JSON.stringify(['intuition', 'feminine_energy', 'lunar_connection', 'emotional_healing']),
